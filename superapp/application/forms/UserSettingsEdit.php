...
class Application_Form_UserSettingsEdit extends Zend_Form
{
    ...
    {
        $userFileformats = $this->_userSettings->getAllowedFileFormats();
        $allFileformats = $this->_allAllowedFileformats;
        ...
        $formelemente = array($userid, $savecloud, $savelocationcloud, $usernamecloud, $passwordcloud, $maxfileupload, $maxstorage);

        foreach ($allFileformats as $allFileformat) {
            $fileformat =  $allFileformat->getFileformat();

            // checkbox for fileformat
            $fileformatCheckbox = new Zend_Form_Element_Checkbox( $fileformat );
            $fileformatCheckbox->setLabel( $fileformat);
            foreach ($userFileformats as  $userFileformat ) {
                if($userFileformat == $fileformat ) {
                    $fileformatCheckbox->setAttribs( array( 'checked' =>  $fileformat ) );
                    break;
                }
            }
            $formelemente[] = $fileformatCheckbox;
        }
        ...
        $formelemente[] = $submitbutton;
        $this->addElements( $formelemente );
    }
}
